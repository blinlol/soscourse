  ...
  )
{
  ...
  // UINTN size = GraphicsOutput->Mode->SizeOfInfo;
  // EFI_GRAPHICS_OUTPUT_MODE_INFORMATION *TmpInfo = AllocatePool(size);
  ...
  //   DEBUG ((DEBUG_INFO, 
  //           "Graphics mode: %d %d\n", 
  //           TmpInfo->HorizontalResolution,
  //           TmpInfo->VerticalResolution));
  // }
  ...
  )
{
  ...
  // LAB 1: Your code here
  // KernelFile = NULL;
  Status = CurrentDriveRoot->Open (CurrentDriveRoot, 
                                   &KernelFile,
                                   KERNEL_PATH,
                                   EFI_FILE_MODE_READ,
                                   0);
